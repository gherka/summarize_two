...
def generate_report(df1, df2, user_dtypes, ridge_spec=None):
    ...
    #Generate static KDE plots for continuous variables to be used in Jinja template
    ...
    for (j, var2) in enumerate(summary["Metadata"]["common_columns"]["Categorical"].keys()):
    ...
    template.stream(summary=summary, cat_plots=cat_diff_plots, ridge_plot=ridge_plot).dump("report.html")