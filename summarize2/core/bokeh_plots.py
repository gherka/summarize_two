...
def generate_xtab_plot(df1, df2, spec):
    ...
    df2_xtab = (
        pd.crosstab(
            ...
            columns=df2[x_col])
                .reset_index()
                .melt(id_vars=y_col, value_name='s'))

    df_xtab = pd.merge(df1_xtab, df2_xtab, how='left')

    #% of total times 360 + 90 because Bokeh starts angle from horizontal
...
def generate_diff_plot(df1, df2, var_name, shade):
    ...
    bokeh_df = pd.DataFrame(
        {'VAR':[str(x).replace("'", "") for x in freq_1[0]],
         'DF1':freq_1[1],
         'DF2':freq_2[1],
...
def generate_ridge_plot(df1, df2, spec):
    ...
    def make_plot(s1, s2, index, shade):
        ...
        p = figure(
            plot_width=965, x_range=(plot_min, plot_max),
            active_scroll="wheel_zoom",
        ...
        hist1 = p.vbar(
            ...
            top=hist1,
            bottom=0,
            fill_color=DF1_COLOR,
            line_color=DF1_COLOR,
            alpha=0.75,
            line_alpha=0.4,
            muted_color=DF1_COLOR,
            muted_alpha=0.1,
        ...
        hist2 = p.vbar(
            ...
            top=hist2,
            bottom=0,
            fill_color=DF2_COLOR,
            line_color=DF2_COLOR,
            alpha=0.75,
            line_alpha=0.4,
            muted_color=DF2_COLOR,
            muted_alpha=0.1,
...
def generate_kde_plot(df1, df2, col_name, shade):
    ...
    #append zeroes to x and y to correctly position the patches
    #as start and end points have to connect
    ...
    p = figure(
        plot_height=170, plot_width=165,
